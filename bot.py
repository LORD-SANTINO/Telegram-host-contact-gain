...
def save_all_sessions():
    data = {}
    for chat_id, client_instance in user_clients.items():
        try:
            data[str(chat_id)] = client_instance.session.save()
...
def load_all_sessions():
    ...
    for chat_id_str, sess_str in user_sessions.items():
        ...
        client_instance = TelegramClient(StringSession(sess_str), API_ID, API_HASH)
        user_clients[chat_id] = client_instance
...
async def handle_message(event):
    ...
    try:
        if state["step"] == "awaiting_api_id":
            ...
            client_instance = TelegramClient(StringSession(), state["api_id"], state["api_hash"])
            user_clients[chat_id] = client_instance
            ...
            try:
                await client_instance.send_code_request(state["phone"])
            ...
            client_instance = user_clients.get(chat_id)
            ...
            try:
                await client_instance.sign_in(state["phone"], text)
            ...
            try:
                await client_instance.sign_in(password=text)
...
async def process_and_store_vcf(event, vcf_path):
    ...
    for i in range(0, len(contacts), batch_size):
        ...
        try:
            result = await client_instance(ImportContactsRequest(contacts=phone_contacts))
...
async def main():
    ...
    load_all_sessions()  # Load saved sessions on startup
